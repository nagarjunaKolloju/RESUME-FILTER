...
ResumeNonProgrammingSkillsMatchedWithJD = []
...
def NonTechnicalSkillScore(resume, jd_txt, progWords = None):
    ...
    for i in range(len(jdSkillMatched)):
        if jdSkillMatched[i].lower() in resume.lower() != -1:
            programmingTotal += 1
            ResumeNonProgrammingSkillsMatchedWithJD.append(jdSkillMatched[i].lower())
...
def skillSetListMatchedWithJD():
    return ResumeProgrammingSkillsMatchedWithJD

def nonTechSkillSetListMatchedWithJD():
    return ResumeNonProgrammingSkillsMatchedWithJD